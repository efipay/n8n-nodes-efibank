...
export async function pixCreateLocation(
  ...
  try {
    ...
    const tipoCob = context.getNodeParameter('tipoCob', index) as "cob" | "cobv";
    const body = {tipoCob: tipoCob};

    const resposta = await efipay.pixCreateLocation({}, body);
    return resposta;
  } catch (error: any) {
