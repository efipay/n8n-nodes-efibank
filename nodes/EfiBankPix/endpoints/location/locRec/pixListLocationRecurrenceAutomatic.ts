...
export async function pixListLocationRecurrenceAutomatic(
  ...
  try {
    ...
    const idRecPresente = context.getNodeParameter('idRecPresente', index) as string;
    ...
    if (idRecPresente && idRecPresente !== 'none') params.idRecPresente = idRecPresente;
    if (convenio?.trim()) params.convenio = convenio;
    ...
    // @ts-ignore
    const resposta = await efipay.pixListLocationRecurrenceAutomatic(params);
    return resposta;
  } catch (error: any) {
