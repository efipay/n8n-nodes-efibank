...
export async function pixUpdateDueCharge(
  ...
  try {
    ...
    const requestBody = context.getNodeParameter('requestBodyPixUpdateDueCharge', index) as string;

    const body = JSON.parse(requestBody);

    const resposta = await efipay.pixUpdateDueCharge({ txid }, body);
    return resposta;
} catch (error: any) {
