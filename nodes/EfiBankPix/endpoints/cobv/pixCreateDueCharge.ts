...
export async function pixCreateDueCharge(
  ...
  try {
    ...
    const requestBody = context.getNodeParameter('requestBodyPixCreateDueCharge', index) as string;

    const body = JSON.parse(requestBody);

    const resposta = await efipay.pixCreateDueCharge({ txid }, body);
    return resposta;
} catch (error: any) {
