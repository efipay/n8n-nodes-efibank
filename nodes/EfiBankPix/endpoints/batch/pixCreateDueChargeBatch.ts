...
export async function pixCreateDueChargeBatch(
  ...
  try {
    ...
    const requestBody = context.getNodeParameter('requestBodyPixCreateDueChargeBatch', index) as string;

    const body = JSON.parse(requestBody);

    await efipay.pixCreateDueChargeBatch({ id }, body);
    return ['code: 202'];
  } catch (error: any) {
