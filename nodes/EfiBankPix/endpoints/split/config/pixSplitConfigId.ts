...
export async function pixSplitConfigId(
  ...
  try {
    ...
    const bodyRequestSplitConfigId = context.getNodeParameter('bodyRequestSplitConfigId', index) as string;
    const splitConfigId = context.getNodeParameter('splitConfigId', index) as string;
    const body = JSON.parse(bodyRequestSplitConfigId);
    
    const resposta = await efipay.pixSplitConfigId({ id: splitConfigId }, body);
    return resposta;
  } catch (error: any) {
