...
export async function pixUpdateCharge(
  ...
  try {
    ...
    const requestBody = context.getNodeParameter('requestBodyPixUpdateCharge', index) as string;

    const body = JSON.parse(requestBody);

    const resposta = await efipay.pixUpdateCharge({ txid }, body);
    return resposta;
  } catch (error: any) {
