...
export async function pixQrCodeDetail(
  ...
  try {
    ...
    const pixCopiaECola = context.getNodeParameter('pixCopiaECola', index) as string;
    const body = {pixCopiaECola: pixCopiaECola};
    
    const resposta = await efipay.pixQrCodeDetail({}, body);
    return resposta;
