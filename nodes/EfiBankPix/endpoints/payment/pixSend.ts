...
export async function pixSend(
  ...
  try {
    ...
    const requestBody = context.getNodeParameter('requestBodyPixSend', index) as string;

    const body = JSON.parse(requestBody);

    const resposta = await efipay.pixSend({ idEnvio }, body);
    return resposta;
  } catch (error: any) {
