...
export async function updateAccountConfig(
  ...
  try {
    ...
    const bodyRequestUpdateAccountConfig = context.getNodeParameter('bodyRequestUpdateAccountConfig', index) as string;
    const body = JSON.parse(bodyRequestUpdateAccountConfig);
    
    const resposta = await efipay.updateAccountConfig({}, body);
    return resposta;
  } catch (error: any) {
