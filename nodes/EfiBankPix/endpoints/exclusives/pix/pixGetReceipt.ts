...
export async function pixGetReceipt(
  ...
  try {
    ...
    const rtrId = context.getNodeParameter('rtrId', index) as string;
    const txid = context.getNodeParameter('txid', index) as string;

    if (e2eid?.trim()) params.e2eid = e2eid;
    if (idEnvio?.trim()) params.idEnvio = idEnvio;
    if (rtrId?.trim()) params.rtrId = rtrId;
    if (txid?.trim()) params.txid = txid;

    const resposta = await efipay.pixGetReceipt(params);
    return resposta;
  } catch (error: any) {
