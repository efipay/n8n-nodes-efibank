...
export async function pixUpdateRecurrenceAutomatic(
  ...
  try {
    ...
    const requestBody = context.getNodeParameter('requestBodyPixUpdateRecurrenceAutomatic', index) as string;

    const body = JSON.parse(requestBody);
    const params = { idRec };
// @ts-ignore
    const resposta = await efipay.pixUpdateRecurrenceAutomatic(params, body);
    return resposta;
  } catch (error: any) {
