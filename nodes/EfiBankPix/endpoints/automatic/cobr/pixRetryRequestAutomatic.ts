...
export async function pixRetryRequestAutomatic(
  ...
  try {
    ...
    const data = context.getNodeParameter('data', index) as string;
...
// @ts-ignore
    const resposta = await efipay.pixRetryRequestAutomatic(params);
    return resposta;
  } catch (error: any) {
