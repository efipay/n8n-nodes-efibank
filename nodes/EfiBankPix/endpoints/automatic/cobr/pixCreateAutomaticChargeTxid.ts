...
export async function pixCreateAutomaticChargeTxid(
  ...
  try {
    ...
    const requestBody = context.getNodeParameter('requestBodyPixCreateAutomaticCharge', index) as string;

    const body = JSON.parse(requestBody);
    const params = { txid };
// @ts-ignore
    const resposta = await efipay.pixCreateAutomaticChargeTxid(params, body);
    return resposta;
  } catch (error: any) {
